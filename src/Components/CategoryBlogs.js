                ...
                if (data) {
                    return (
                        <Link to={`/blogs/${item.slug}`} style={{ textDecoration: "none" }} key={index}>
                            <BlogCards title={item.title} categories={item.tags} image={item.coverImage.gatsbyImageData} time={item.createdAt} description={item.shortDescription} readingTime={item.fields.timeToRead} index={index} />
                        </Link>
                    )
                }
                else{
